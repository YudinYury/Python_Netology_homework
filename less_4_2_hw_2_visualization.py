...
def ruth_n_robert_each_5year():
    global source_dir_path
    names = []
    names_dict = {}
    for i in range(1900, 1921, 5):
        names_dict[i] = download_year_data(i)
    ruth_n_robert_each_5y = pd.concat(names_dict, names=['Year'])
    ruth_n_robert_each_5y.unstack('Name').plot.bar()
